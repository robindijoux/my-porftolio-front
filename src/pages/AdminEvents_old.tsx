...
const AdminEvents = () => {
  ...
  const { events, deleteEvent, exportEvents, importEvents, loadDynamicEvents } = useTimelineEvents();
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground">Travail</p>
                  <p className="text-2xl font-bold">{events.filter(e => e.type === 'work').length}</p>
                </div>
                <div className="h-8 w-8 rounded-full bg-green-500/10 flex items-center justify-center">
                  <span className="text-green-600 text-sm font-bold">T</span>
        ...
        {sortedEvents.length === 0 ? (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-muted-foreground mb-4">
                Vous n'avez pas encore créé d'événements pour votre timeline.
              </p>
              <Button asChild>
                <Link to="/create-event">
                  <Plus className="h-4 w-4 mr-2" />
